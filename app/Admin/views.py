...
@admin_login_req
def file_list():
    files = [f for f in os.listdir(current_app.config['UPLOAD_FOLDER'])if os.path.isfile(
        os.path.join(current_app.config['UPLOAD_FOLDER'], f))]
    files_list = []
    for f in files:
        size = os.path.getsize(os.path.join(
            current_app.config['UPLOAD_FOLDER'], f))/1024
        files_list.append({'name': f, 'size': int(size), 'url': '/file/'+f,
                           'thumb_url': '/thumb_file/'+f, 'delete_url': '/delete/'+f})
